...
See EXAMPLES.md for detailed flow implementations.
...
| File | Purpose |
|------|---------|
| src/server/client.ts | `Auth0Client` implementation |
| src/server/auth-client.ts | Internal OAuth/route handling |
| src/client/hooks/use-user.ts | `useUser` hook |
| src/errors/index.ts | All error class definitions |
