...
* **Spread the word**. Evangelize MUI's products by linking to mui.com on your website—every backlink matters. Follow us on X, like and retweet the important news. Or just talk about us with your friends.
...
Visit the guide about integration with third-party routing libraries, like react-router or Next.js, for more details.
...
If you're not sure if the Material UI component in question forwards its ref you can check the API documentation under "Props." You should find the message below, like in the Button API.
...
Components like the Portal or Popper require a DOM node in the `container` or `anchorEl` prop respectively. It seems convenient to simply pass a ref object in those props and let Material UI access the current value.
...
The templates are built using a shared theme. Below are the structure to create a new template:
...
If you have several applications running on one page, consider using one @mui/styles module for all of them. If you are using webpack, you can use the `splitChunks` configuration to create a shared vendor chunk that will contain the @mui/styles module:
...
The #1 reason this happens is likely due to class name conflicts once your code is in a production bundle. For Material UI to work, the `className` values of all components on a page must be generated by a single instance of the class name generator.
