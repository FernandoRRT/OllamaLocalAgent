...
To animate a component when it is mounted or unmounted, you can use the `TransitionGroup` component from _react-transition-group_. As components are added or removed, the `in` prop is toggled automatically by `TransitionGroup`.
...
For more information on creating a custom transition, visit the _react-transition-group_`Transition` documentation. You can also visit the dedicated sections of some of the components:
