...
// O seu array extraído com precisão! (A propriedade 'file' será ignorada para evitar colisões)
const NEXT_DOCS = [
...
async function fetchAndClean(targetUrl) {
  console.log(`📥 Baixando: ${targetUrl}...`);
  try {
    ...
    // 5. LIMPEZA DE ESPAÇOS E TRAÇOS ÓRFÃOS
    md = md.replace(/^\*\s+$/gm, ''); 
    md = md.replace(/\n{3,}/g, '\n\n'); 

    // GERANDO NOME DE ARQUIVO SEGURO
    const urlObj = new URL(targetUrl);
    // Pega o path, ignora os trechos repetitivos e vazios
    ...
    console.log(`✅ Salvo em: ${safeFileName}\n`);

  } catch (error) {
    console.error(`❌ Erro em ${targetUrl}:`, error.message);
  }
}

async function runBulk() {
  console.log("🚀 Iniciando Bulk Fetch da Stack Next.js (117 arquivos)...\n");
  
  // Limpa o diretório antigo primeiro por segurança
  ...
  for (const doc of NEXT_DOCS) {
    ...
    // IMPORTANTE: Mantém o delay de 1s para não ser bloqueado pela API do Jina.
    // O processo vai levar cerca de 2 minutos para rodar inteiro.
    await new Promise(resolve => setTimeout(resolve, 1000));
  }
  
  console.log("🎉 Download em lote concluído! Execute o comando cat para unificar.");
}
