...
## How PPR Works[](https://nextjs.org/docs/app/guides/ppr-platform-guide#how-ppr-works)

### Build time[](https://nextjs.org/docs/app/guides/ppr-platform-guide#build-time)

For each PPR route, Next.js produces:

*   A **static HTML shell** containing all the content that can be prerendered, with Suspense fallbacks where dynamic content will appear.
...
### Request time[](https://nextjs.org/docs/app/guides/ppr-platform-guide#request-time)
...
## Storing PPR Artifacts[](https://nextjs.org/docs/app/guides/ppr-platform-guide#storing-ppr-artifacts)
...
These must be stored and updated atomically. When a PPR route is revalidated (via time-based or on-demand revalidation), Next.js regenerates both the shell and the postponed state together. Serving a new shell with an old postponed state, or vice versa, will produce incorrect dynamic content.

Use `requestMeta.onCacheEntryV2` in your adapter to observe cache updates and propagate them to your storage backend.

## Origin-Only Implementation[](https://nextjs.org/docs/app/guides/ppr-platform-guide#origin-only-implementation)
...
## CDN Shell + Origin Compute[](https://nextjs.org/docs/app/guides/ppr-platform-guide#cdn-shell--origin-compute)
...
## The Resume Protocol[](https://nextjs.org/docs/app/guides/ppr-platform-guide#the-resume-protocol)
...
### CDN-to-origin[](https://nextjs.org/docs/app/guides/ppr-platform-guide#cdn-to-origin)
...
### Adapter-based[](https://nextjs.org/docs/app/guides/ppr-platform-guide#adapter-based)
...
### Finding PPR routes in build output[](https://nextjs.org/docs/app/guides/ppr-platform-guide#finding-ppr-routes-in-build-output)

In the adapter output, PPR routes are identified by `renderingMode: 'PARTIALLY_STATIC'` in the prerenders array. Iterate `outputs.prerenders` to find these entries and read `fallback.postponedState`.

`pprChain.headers` contains the headers needed for the resume protocol: `{ 'next-resume': '1' }`.

For detailed implementation with code examples, see Implementing PPR in an Adapter.

## Implementation Checklist[](https://nextjs.org/docs/app/guides/ppr-platform-guide#implementation-checklist)
...
For the complete adapter API reference and implementation examples, see the Deployment Adapter API.

Related guides and references.

### Rendering Philosophy Learn how Next.js treats static and dynamic rendering as a spectrum at the component level, and what this means for deployment.### Streaming Learn how streaming works in Next.js and how to use it to progressively render UI as data becomes available.### Deploying to Platforms Understand which Next.js features require specific platform capabilities and how to choose the right deployment target.### adapterPath Configure a custom adapter for Next.js to hook into the build process.

Previous Package BundlingNext Prefetching