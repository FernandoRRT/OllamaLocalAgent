...
Client-side transitions are what makes a server-rendered apps _feel_ like client-rendered apps. And when paired with prefetching and streaming, it enables fast transitions, even for dynamic routes.

Next.js also handles scrolling to the top of the page during client-side transitions. If content scrolls behind a sticky or fixed header after navigation, you can fix this with CSS `scroll-padding-top`.
...
### Link Component Enable fast client-side navigation with the built-in `next/link` component.### loading.js API reference for the loading.js file.### Prefetching Learn how to configure prefetching in Next.js

Previous Layouts and PagesNext Server and Client Components