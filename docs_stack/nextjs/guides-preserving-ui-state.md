...
> **Good to know:** This guide assumes Cache Components is enabled. Enable it by setting `cacheComponents: true` in your Next config file.
...
Instead of unmounting pages on navigation, Next.js hides them using React's `<Activity>` component. The DOM nodes stay in the document (hidden with `display: none`), so both React state and DOM state are preserved: form drafts, scroll positions, expanded `<details>` elements, video playback progress, and more.
...
## Choosing what to preserve[](https://nextjs.org/docs/app/guides/preserving-ui-state#choosing-what-to-preserve)
...
### Expandable UI (dropdowns, accordions, panels)[](https://nextjs.org/docs/app/guides/preserving-ui-state#expandable-ui-dropdowns-accordions-panels)
...
You can also use `Link`'s `onNavigate` callback to close dropdowns immediately when a navigation link is clicked.

### Dialog and initialization logic[](https://nextjs.org/docs/app/guides/preserving-ui-state#dialog-and-initialization-logic)
...
### Form input values[](https://nextjs.org/docs/app/guides/preserving-ui-state#form-input-values)
...
### Action state (`useActionState`)[](https://nextjs.org/docs/app/guides/preserving-ui-state#action-state-useactionstate)

Activity preserves `useActionState` results: success messages, error messages, and any other state returned by the action.
...
You can think of `useActionState` as a `useReducer` that allows side effects. It doesn't have to only handle form submissions; you can dispatch any action to it. Adding a `RESET` action gives you a clean way to clear state when Activity hides the component (see Reset state in the React docs):
...
## State and authentication[](https://nextjs.org/docs/app/guides/preserving-ui-state#state-and-authentication)
...
## Global styles[](https://nextjs.org/docs/app/guides/preserving-ui-state#global-styles)
...
## Testing[](https://nextjs.org/docs/app/guides/preserving-ui-state#testing)
...
### Use visibility-aware selectors[](https://nextjs.org/docs/app/guides/preserving-ui-state#use-visibility-aware-selectors)
...
## Using Activity in your components[](https://nextjs.org/docs/app/guides/preserving-ui-state#using-activity-in-your-components)
...
### Prerendering hidden content[](https://nextjs.org/docs/app/guides/preserving-ui-state#prerendering-hidden-content)
...
### Effect and media cleanup[](https://nextjs.org/docs/app/guides/preserving-ui-state#effect-and-media-cleanup)
...
### Distinguishing first mount from re-show[](https://nextjs.org/docs/app/guides/preserving-ui-state#distinguishing-first-mount-from-re-show)
...
## Examples[](https://nextjs.org/docs/app/guides/preserving-ui-state#examples)

The Activity Patterns Demo (source) is a Next.js app with Cache Components enabled and three routes. Navigate between them to see state preservation in action:
...
Previous PrefetchingNext Production