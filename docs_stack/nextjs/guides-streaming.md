...
## What is streaming?[](https://nextjs.org/docs/app/guides/streaming#what-is-streaming)
...
## Example[](https://nextjs.org/docs/app/guides/streaming#example)
...
## How the App Router delivers a page[](https://nextjs.org/docs/app/guides/streaming#how-the-app-router-delivers-a-page)

When a browser requests a page, two streams work together during the initial page load:

### The HTML stream[](https://nextjs.org/docs/app/guides/streaming#the-html-stream)
...
### The component payload[](https://nextjs.org/docs/app/guides/streaming#the-component-payload)
...
### The static shell[](https://nextjs.org/docs/app/guides/streaming#the-static-shell)
...
## Page-level streaming with `loading.js`[](https://nextjs.org/docs/app/guides/streaming#page-level-streaming-with-loadingjs)
...
## Granular streaming with `<Suspense>`[](https://nextjs.org/docs/app/guides/streaming#granular-streaming-with-suspense)
...
### Parallel streaming with sibling boundaries[](https://nextjs.org/docs/app/guides/streaming#parallel-streaming-with-sibling-boundaries)
...
### Nested boundaries for progressive detail[](https://nextjs.org/docs/app/guides/streaming#nested-boundaries-for-progressive-detail)
...
### Push dynamic access down[](https://nextjs.org/docs/app/guides/streaming#push-dynamic-access-down)
...
### When to use `loading.js` vs `<Suspense>`[](https://nextjs.org/docs/app/guides/streaming#when-to-use-loadingjs-vs-suspense)
...
### Error handling mid-stream[](https://nextjs.org/docs/app/guides/streaming#error-handling-mid-stream)
...
## Streaming data to the client[](https://nextjs.org/docs/app/guides/streaming#streaming-data-to-the-client)
...
### Sharing a promise across the tree[](https://nextjs.org/docs/app/guides/streaming#sharing-a-promise-across-the-tree)
...
## Streaming in Route Handlers[](https://nextjs.org/docs/app/guides/streaming#streaming-in-route-handlers)
...
## Streaming and Web Vitals[](https://nextjs.org/docs/app/guides/streaming#streaming-and-web-vitals)

Web Vitals are the metrics Google uses to measure user experience. Streaming directly affects several of them.

### TTFB and FCP[](https://nextjs.org/docs/app/guides/streaming#ttfb-and-fcp)
...
### LCP (Largest Contentful Paint)[](https://nextjs.org/docs/app/guides/streaming#lcp-largest-contentful-paint)
...
### CLS (Cumulative Layout Shift)[](https://nextjs.org/docs/app/guides/streaming#cls-cumulative-layout-shift)
...
### INP (Interaction to Next Paint)[](https://nextjs.org/docs/app/guides/streaming#inp-interaction-to-next-paint)

Streaming enables selective hydration: React hydrates components independently as they stream in, and prioritizes hydrating whatever the user is interacting with. Each `<Suspense>` boundary is a hydration unit. Without them, React hydrates the entire page in one blocking pass. With them, hydration is broken into smaller tasks that yield to the browser, keeping the main thread responsive. The companion demo lets you compare a single blocking hydration pass with split hydration using Suspense boundaries.

### Early resource discovery[](https://nextjs.org/docs/app/guides/streaming#early-resource-discovery)
...
In the dashboard example above, the `<h1>` renders in the shell (good for LCP), each data section streams independently behind its own Suspense boundary (good for INP since hydration is split), and the skeleton fallbacks reserve space (good for CLS).

## The HTTP contract[](https://nextjs.org/docs/app/guides/streaming#the-http-contract)
...
### Status codes[](https://nextjs.org/docs/app/guides/streaming#status-codes)

When a `<Suspense>` fallback renders or a component suspends, the server must commit to `200 OK` in order to start sending the HTML stream. If a `notFound()` fires mid-stream, Next.js cannot go back and change the status to 404. Instead, it injects `<meta name="robots" content="noindex">` into the streamed HTML so that search engines don't index the page. Similarly, a `redirect()` mid-stream becomes a client-side redirect rather than an HTTP redirect header.

### When does streaming start?[](https://nextjs.org/docs/app/guides/streaming#when-does-streaming-start)
...
> **Good to know:** You can also reject requests early using `proxy` (for redirects, rewrites, or returning a response) or `next.config.js` redirects. Both run before the page renders, so HTTP status codes are still available.

### Metadata and bots[](https://nextjs.org/docs/app/guides/streaming#metadata-and-bots)
...
See the `loading.js` SEO section for more details.

## What can affect streaming[](https://nextjs.org/docs/app/guides/streaming#what-can-affect-streaming)
...
### Reverse proxies[](https://nextjs.org/docs/app/guides/streaming#reverse-proxies)
...
### CDNs[](https://nextjs.org/docs/app/guides/streaming#cdns)
...
### Serverless platforms[](https://nextjs.org/docs/app/guides/streaming#serverless-platforms)
...
### Compression[](https://nextjs.org/docs/app/guides/streaming#compression)
...
### Clients[](https://nextjs.org/docs/app/guides/streaming#clients)

Buffering also happens at the client. Safari/WebKit buffers streaming responses until 1024 bytes have been received, so very small responses paint all at once instead of progressively. Real applications easily exceed this threshold (layouts, styles, scripts), so it only affects minimal demos or tiny Route Handler responses.
...
### Verifying that streaming works[](https://nextjs.org/docs/app/guides/streaming#verifying-that-streaming-works)

This section is about confirming the HTTP response is actually arriving in chunks through your infrastructure. For guidance on designing meaningful loading states and placing Suspense boundaries effectively, see Granular streaming with `<Suspense>` and the Cache Components guide.
...
Run with `node stream-observer.mjs`. For a page with two sibling Suspense boundaries (like the companion demo's Suspense page), you will see output similar to:
...
### Platform support[](https://nextjs.org/docs/app/guides/streaming#platform-support)
...
See the Self-Hosting guide for detailed configuration instructions.

## Summary[](https://nextjs.org/docs/app/guides/streaming#summary)
...
The key decisions are **what to cache** and **where to place Suspense boundaries**. Cache what you can with `"use cache"` to grow the static shell. Push dynamic access down to the components that need it, and wrap those in `<Suspense>`. Everything else becomes part of the shell.

## Further reading[](https://nextjs.org/docs/app/guides/streaming#further-reading)

*   RSC Explorer - interactive tool to explore the component payload format and see how React reconstructs the tree from streamed chunks
*   Streams API on web.dev - introduction to the Web Streams API that underpins streaming in Route Handlers
*   Chunked transfer encoding (MDN) - the HTTP/1.1 mechanism that enables streaming responses
*   browser.engineering - deep dive into how browsers handle network responses, rendering, and progressive display

Related API references and guides.

### loading.js API reference for the loading.js file.### Fetching Data Learn how to fetch data and stream content that depends on data.### Linking and Navigating Learn how the built-in navigation optimizations work, including prefetching, prerendering, and client-side navigation, and how to optimize navigation for dynamic routes and slow networks.### Self-Hosting Learn how to self-host your Next.js application on a Node.js server, Docker image, or static HTML files (static exports).### Rendering Philosophy Learn how Next.js treats static and dynamic rendering as a spectrum at the component level, and what this means for deployment.

Previous Static ExportsNext Tailwind CSS v3