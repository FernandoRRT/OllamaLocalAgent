...
You can use rewrites with header matching to serve different content types from the same URL based on the request's `Accept` header. This is known as content negotiation.
...
### route.js API reference for the route.js special file.### proxy.js API reference for the proxy.js file.### rewrites Add rewrites to your Next.js app.

Previous AuthenticationNext Caching (Previous Model)