...
## Example[](https://nextjs.org/docs/app/guides/view-transitions#example)
...
### Step 1: Morph a thumbnail into a hero image[](https://nextjs.org/docs/app/guides/view-transitions#step-1-morph-a-thumbnail-into-a-hero-image)
...
#### Customizing the morph animation[](https://nextjs.org/docs/app/guides/view-transitions#customizing-the-morph-animation)

To control the morph CSS, add `share="morph"`. This assigns the `morph` class to the view transition, which you can target with CSS pseudo-elements. For example, to soften the morph mid-flight with a `blur` keyframe:
...
### Step 2: Animate loading states with Suspense reveals[](https://nextjs.org/docs/app/guides/view-transitions#step-2-animate-loading-states-with-suspense-reveals)
...
### Step 3: Add directional motion for navigation[](https://nextjs.org/docs/app/guides/view-transitions#step-3-add-directional-motion-for-navigation)
...
#### Anchoring the header[](https://nextjs.org/docs/app/guides/view-transitions#anchoring-the-header)
...
#### Respecting reduced motion[](https://nextjs.org/docs/app/guides/view-transitions#respecting-reduced-motion)
...
Without animation, content swaps instantly, which is the browser's default behavior. A more refined approach would preserve crossfades and opacity transitions while removing positional movement. See "No Motion Isn't Always prefers-reduced-motion" for more on this.

### Step 4: Crossfade content within the same route[](https://nextjs.org/docs/app/guides/view-transitions#step-4-crossfade-content-within-the-same-route)
...
## Next steps[](https://nextjs.org/docs/app/guides/view-transitions#next-steps)
...
*   `useRouter` — also supports `transitionTypes` in `push()` and `replace()`
*   React `ViewTransition` component
*   Complete CSS from this guide — all keyframes and view transition rules in one file

Previous VideosNext API Reference